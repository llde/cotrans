...
function mount(): TranslatorInstance {
  ...
  function findImageNodes(node: HTMLElement) {
    ...
    .filter(node => node.classList.contains('ts-main-image')
    )
  }
  ...
  function createInstance(imageNode: HTMLImageElement): Omit<Instance, 'dispose'> {
    ...
    imageNode.parentElement.style.position = 'relative'
    const container = document.createElement('div')
 //   container.appendChild(imageNode)
    container.style.top = imageNode.offsetTop+'px'
    container.style.position = "absolute"
    imageNode.parentElement.appendChild(container)
    console.log(imageNode.offsetTop)
    onCleanup(() => {
      container.remove()
    })
    ...
    async function getTranslatedImage(optionsOverwrite?: TranslateOptionsOverwrite): Promise<string> {
      ...
      if (!originalImage) {
        ...
        const result = await GMP.xmlHttpRequest({
          ...
          responseType: 'blob',
          url: originalSrc,
          headers: { referer: 'https://www.rawkuma.com/' },
    ...
    async function enable(optionsOverwrite?: TranslateOptionsOverwrite) {
      try {
        ...
        imageNode.setAttribute('data-trans', originalSrc)
...
const translator: Translator = {
  match(url) {
    return url.hostname.endsWith('rawkuma.com') && !url.pathname.match(/\/manga\//)
  },
  mount,
}
